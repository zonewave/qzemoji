...
`[em]e{数字}[/em]` 格式的消息代码。除了供用户查找、复制并在 K 歌聊天中发送，还可以
作为机器人消息配置或自动回复的数据源。例如，EID `1000004` 的消息代码为：
...
1. 在仓库的 [GitHub Releases](https://github.com/zonewave/qzemoji/releases) 页面下载纯数字版
...
如果机器人支持发送 K 歌聊天消息，可以将图册中的完整消息代码配置为固定回复、关键词回复或
随机表情内容。需要动态选择表情时，也可以读取 `emoji.db` 中的 EID，并按
`[em]e{eid}[/em]` 格式生成消息。
